terraform {
  backend "s3" {
    bucket = "tfstate.pios.example.com"
    ...
    skip_metadata_api_check = true #EC2 Metadata API. trueならチェックしません。
    force_path_style = true # true なら https://<HOST>/<BUCKET> とします。
  }
}
...
provider "openstack" {
  ...
  user_name   = "taira"
  password    = "taira123"
  auth_url    = "http://pios01.example.com:5000/v3/"
}
